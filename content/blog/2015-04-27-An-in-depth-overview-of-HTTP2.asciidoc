---
layout: project-blog
title: An in depth overview of HTTP/2
author: Stuart Douglas
email: <sdouglas@redhat.com>
date: 2015-03-26
revision: 1.0
aliases: [/blog/2015/04/27/An-in-depth-overview-of-HTTP2.html]
---
...
ALPN based connection for https:// URIs::
    ...
    https://tools.ietf.org/html/rfc7301[ALPN] stands for 'Application Layer Protocol Negotiation', and is a TLS extension that allows a client to negotiate the
...
This post will not go into an in-depth discussion of the wire format, instead we will just give an overview of the basic
concepts (if you want more details I highly recommend going directly to the https://http2.github.io/[specification]).
...
As a result there are lots of workarounds that are used on both the server and browser side to improve performance, including:
...
+:authority+::
Similar to the HTTP/1.1 +Host:+ header, this contains the authority portion of the target URI (e.g. https://example.com)
...
Pushed resources must be https://tools.ietf.org/html/rfc7234[cacheable]. In practice this means that pushed requests
