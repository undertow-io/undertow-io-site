---
title: Undertow Handler Authors Guide
layout: doc
---
...
This guide provides an overview of how to write native handlers for Undertow. Lets start with a simple example:
...
public class HelloWorldServer {
    ...
    public static void main(final String[] args) {
        Undertow server = Undertow.builder()                                                    //Undertow builder
                ...
                .setDefaultHandler(new HttpHandler() {                                          //Default Handler
                    @Override
...
Note that the sender API does not support queueing, you may not call `send()` again until after the callback has been
notified.
...
`HttpServerExchange.startBlocking(BlockingHttpServerExchange blockingExchange)` which allows you to customise the
